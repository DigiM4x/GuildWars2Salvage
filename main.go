...
var templates = template.Must(template.ParseFiles("index.html", "addSalvage.html", "viewSalvage.html"))
...
// Handler to show static files that don't need processing
func sourceHandler(response http.ResponseWriter, request *http.Request) {
...
func indexHandler(response http.ResponseWriter, requeest *http.Request) {
...
func viewSalvageDataHandler(response http.ResponseWriter, request *http.Request) {
	...
	for _, item := range salvages {
		...
		go func(salvageItem Salvage) {
			...
			handleError(err, response, "Unable to Unmarshal contents of response to "+requestURL+strconv.Itoa(salvageItem.ID))
...
func main() {
	...
	http.HandleFunc("/gw2library.js", sourceHandler)
	http.HandleFunc("/gw2.css", sourceHandler)
